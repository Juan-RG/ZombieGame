...
public class EnemyZombiRed extends Enemy{
	//estado del enemigo
	public State state;
	...
	//puntuacion zombi roji
	int puntuacionZombiRojo = 50;
	...
	//constructor
	public EnemyZombiRed(SpriteManager spriteManager) {
		super(spriteManager);
		...
		TextureAtlas atlas=ResourceManager.assets.get("characters/zombi_red/zombi_red.pack", TextureAtlas.class);
	...
	public void checkCollisions(SpriteManager spriteManager) {
		...
		for (int i = 0; i < spriteManager.enemys.size;i++) {
			...
			if(spriteManager.enemys.get(i).getClass().getName().equals("com.mygdx.zombi.characters.EnemyZombiRed")){
	...
	public void restarVida(int damage){
		...
		if (this.vida<=0) {
			this.spriteManager.hud.addScore(puntuacionZombiRojo);
