...
public class EnemyZombi extends Enemy{
	//estado del enemigo
	public State state;
	...
	//Estados del enemigo
	public enum State{
		IDLE, RUNRIGHT, RUNLEFT, RUNUP, RUNDOWN, ATTACKRIGHT, ATTACKLEFT, ATTACKUP, ATTACKDOWN, DEADUP, DEADDOWN, DEADRIGHT, DEADLEFT
	}

	//constructor
	public EnemyZombi(SpriteManager spriteManager) {
		super(spriteManager);
	...
	public void checkCollisions(SpriteManager spriteManager) {
		...
		for (int i = 0; i < spriteManager.enemys.size;i++) {
			EnemyZombi enemy = null;
			
			if(spriteManager.enemys.get(i).getClass().getName().equals("com.mygdx.zombi.characters.EnemyZombi")){
				enemy = (EnemyZombi) spriteManager.enemys.get(i);
			...
			if(!enemy.equals(this)) {
				...
				EnemyZombiRed enemynormal = null;
				enemynormal = (EnemyZombiRed) spriteManager.enemys.get(i);
			...
			}else if(spriteManager.enemys.get(i).getClass().getName().equals("com.mygdx.zombi.characters.EnemyZombiExplosivo")){
				EnemyZombiExplosivo enemynormal = null;
				enemynormal = (EnemyZombiExplosivo) spriteManager.enemys.get(i);
	...
	public void restarVida(int damage){
		...
		if (this.vida<=0) {
			this.spriteManager.hud.addScore(puntuacionZombi);
